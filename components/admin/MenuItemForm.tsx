...
export function MenuItemForm({ categories, item }: Props) {
  ...
  const doSave = async () => {
    ...
    try {
      ...
      if (!currentId) {
        ...
        const updateData = imageRemoved && !pendingImageFile
  ...
  }, [name, description, priceCents, categoryId, pendingImageFile, imageRemoved])
  ...
  return (
    <div className="flex flex-col gap-6 max-w-xl">
      ...
      {/* Price */}
      <div className="flex flex-col gap-1">
        <label htmlFor="price-input" className="text-sm font-medium text-text-primary">Price</label>
      ...
      {/* Image */}
      <div className="flex flex-col gap-1">
        <label htmlFor="image-input" className="text-sm font-medium text-text-primary">Image</label>
        <div
          onDrop={handleDrop}
        ...
        >
          {imagePreview ? (
            <img src={imagePreview} alt="Preview" className="mx-auto max-h-40 rounded-md object-cover" />
          ) : (
            <p className="text-sm text-text-tertiary">Drag & drop or tap to select</p>
          )}
          <input
            key={fileInputKey}
            id="image-input"
      ...
      <div className="text-sm min-h-[1.25rem]">
        ...
        {saveStatus === 'error' && saveError && (
          <p role="alert" className="text-red-500">{saveError}</p>
        )}
      </div>
      ...
      {item && (
        <div className="flex gap-4 text-sm border-t border-border pt-4">
          <Link href="/admin/menu/new" className="text-text-secondary hover:text-accent">
            Add another item →
          </Link>
          <Link href="/admin/menu" className="text-text-secondary hover:text-accent">
            Back to menu
          </Link>
        </div>
      )}
